...
namespace blueprint
{
    /** The CanvasContext class is a C++ implementation/binding for the
        ...
        @endcode
    */
    class CanvasContext final : public juce::DynamicObject
    {
    public:
        //==============================================================================
        /** */
        ...
        /** Set the size of the CanvasContext.

            This is akin to setting the width and height attributes on a HTML <canvas> element.

            If you have a blueprint::View or a juce::Component that wished to render
            a CanvasContext you should ensure CanvasContext::setSize()
            is called from the View/Component's resized function.
        */
        ...
        /** @returns the juce::Image instance that has been rendered to.
            This can be drawn to the screen using juce::Graphics::drawImageAt().
        */
        const juce::Image& getImage() const { return image; }

    private:
        //==============================================================================
        int ctxWidth = 1, ctxHeight = 1;
        juce::Image image;
        ...
        void registerNativeProperties()
        {
            setProperty ("__setFillStyle", juce::var::NativeFunction
            {
                [=] (const juce::var::NativeFunctionArgs& args) -> juce::var
                {
                    ...
                    properties.fillStyle.setColour (juce::Colour::fromString (args.arguments[0].toString()));
            ...
            setProperty ("__setStrokeStyle", juce::var::NativeFunction
            {
                [=] (const juce::var::NativeFunctionArgs& args) -> juce::var
                {
                    ...
                    properties.strokeStyle.setColour (juce::Colour::fromString (args.arguments[0].toString()));
            ...
            setProperty ("__setLineWidth", juce::var::NativeFunction
            {
                [=] (const juce::var::NativeFunctionArgs& args) -> juce::var
                {
                    ...
                    properties.lineWidth = static_cast<int> (args.arguments[0]);
            ...
            */
            ...
            setProperty ("__setTextAlign", juce::var::NativeFunction
            {
                [=] (const juce::var::NativeFunctionArgs& args) -> juce::var
                {
                    ...
                    const auto textAlign = args.arguments[0].toString();
                    ...
                    if (textAlign == "left")        properties.textAlign = juce::Justification::left;
                    else if (textAlign == "right")  properties.textAlign = juce::Justification::right;
                    else if (textAlign == "center") properties.textAlign = juce::Justification::horizontallyCentred;
                    else if (textAlign == "start")  properties.textAlign = juce::Justification::left;
                    else if (textAlign == "end")    properties.textAlign = juce::Justification::right;
        ...
        void registerNativeFunctions()
        {
            ...
            setProperty ("drawImage", juce::var::NativeFunction
            {
                [=] (const juce::var::NativeFunctionArgs& args) -> juce::var
                {
                    ...
                    const auto svg = args.arguments[0].toString();
                    ...
                    auto svgElement = juce::XmlDocument::parse (svg);
                    ...
                    if (auto svgDrawable = juce::Drawable::createFromSVG (*svgElement))
                    {
                        if (args.numArguments == 5)
                        {
                            ...
                            svgDrawable->setTransformToFit ({ xPos, yPos, destWidth, destHeight },
            ...
            setProperty ("strokeText", juce::var::NativeFunction {
                [=] (const juce::var::NativeFunctionArgs& args) -> juce::var {
                    ...
                    const auto text = args.arguments[0].toString();
                    const float xPos = args.arguments[1];
                    const float yPos = args.arguments[2];

                    // Default maxLineWidth to full context width
                    auto maxLineWidth = (float)ctxWidth;
        ...
        //==============================================================================
        JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (CanvasContext)
    };
    ...
    */
    class CanvasView final : public View,
                             private juce::Timer
    {
    public:
        //==============================================================================
        ...
        void setProperty (const juce::Identifier& name, const juce::var& value) override
        {
            ...
            {
                const bool shouldAnimate = static_cast<bool> (value);
    ...
    private:
        //==============================================================================
        juce::ReferenceCountedObjectPtr<CanvasContext> ctx { new CanvasContext() };
