namespace blueprint
{
    namespace detail
    {
        ...
        static juce::String getContextDump (duk_context* ctx)
        {
            ...
            const auto ret = juce::String (duk_to_string (ctx, -1));
            duk_pop (ctx);
            return ret;
        }
        ...
        static void safeCall (duk_context* ctx, const int numArgs)
        {
            ...
            {
                const auto stack = juce::String (duk_safe_to_stacktrace (ctx, -1));
                const auto msg = juce::String (duk_safe_to_string (ctx, -1));
    ...
    EcmascriptEngine::EcmascriptEngine()
    {
        ...
        dukContext = std::shared_ptr<duk_context> (duk_create_heap (nullptr, nullptr, nullptr, nullptr, detail::fatalErrorHandler), duk_destroy_heap);
        ...
        duk_push_pointer (ctxRawPtr, this);
    ...
    void EcmascriptEngine::debuggerAttach()
    {
        ...
        duk_debugger_attach (ctxRawPtr,
            ...
            [] (duk_context*, void* data)
            {
                ...
                static_cast<EcmascriptEngine*> (data)->stopTimer();
            },
            this);
    ...
    duk_ret_t EcmascriptEngine::LambdaHelper::invokeFromDukContext (duk_context* ctx)
    {
        ...
        auto* helper = static_cast<LambdaHelper*> (duk_get_pointer (ctx, -1));
        duk_pop (ctx);
    ...
    void EcmascriptEngine::pushVarToDukStack (std::shared_ptr<duk_context> ctx, const juce::var& v, bool persistNativeFunctions)
    {
        ...
        if (v.isVoid() || v.isUndefined())  { duk_push_undefined (ctxRawPtr); return; }
        if (v.isBool())                     { duk_push_boolean (ctxRawPtr, (bool) v); return; }
        if (v.isInt() || v.isInt64())       { duk_push_int (ctxRawPtr, (int) v); return; }
        if (v.isDouble())                   { duk_push_number (ctxRawPtr, (double) v); return; }
        if (v.isString())                   { duk_push_string (ctxRawPtr, v.toString().toRawUTF8()); return; }

        if (v.isArray())
        {
            const auto arr_idx = duk_push_array (ctxRawPtr);
            duk_uarridx_t i = 0;
        ...
        if (v.isObject())
        {
            ...
            {
                const auto obj_idx = duk_push_object (ctxRawPtr);
    ...
    juce::var EcmascriptEngine::readVarFromDukStack (std::shared_ptr<duk_context> ctx, duk_idx_t idx)
    {
        ...
        switch (duk_get_type (ctxRawPtr, idx))
        {
            ...
            case DUK_TYPE_UNDEFINED:    value = juce::var::undefined(); break;
            case DUK_TYPE_BOOLEAN:      value = (bool) duk_get_boolean (ctxRawPtr, idx); break;
            case DUK_TYPE_NUMBER:       value = duk_get_number (ctxRawPtr, idx); break;
            case DUK_TYPE_STRING:       value = juce::String (juce::CharPointer_UTF8 (duk_get_string (ctxRawPtr, idx))); break;
            ...
            case DUK_TYPE_LIGHTFUNC:
            {
                ...
                {
                    const auto len = duk_get_length (ctxRawPtr, idx);
